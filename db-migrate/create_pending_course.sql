CREATE TABLE IF NOT EXISTS `pending_course` (
  ...
  `course_type` INT NOT NULL COMMENT '-2体验未成单,-1体验成单,0订场,1班课,2私教,3单次班课',
  `participant_count` INT NOT NULL DEFAULT 0 COMMENT '单次班课上报人数，其他类型为0',
  `is_adult` INT NULL DEFAULT 1 COMMENT '沿用course.is_adult语义',
  `description` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '课程备注',
  `members_data` JSON NOT NULL COMMENT '会员消费数组，体验课和单次班课为[]',
