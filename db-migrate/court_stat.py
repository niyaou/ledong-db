...
def main():
    ...
    try:
        ...
        spend_result_df = spark.sql("""
            ...
            WHERE court.name = '领馆国际校区'
            ORDER BY c.start_time DESC
        """)
        
        # 查询充值记录：直接通过charge表的court字段筛选
        charge_result_df = spark.sql("""
            SELECT 
                ch.id as `充值ID`,
                ch.charge as `充值金额`,
                ch.times as `次卡充值`,
        ...
        if len(spend_pandas_df) == 0 and len(charge_pandas_df) == 0:
            logger.warning("未找到领馆国际校区的消费和充值记录")
            print("\n未找到领馆国际校区的消费和充值记录\n")
            return
        ...
        output_file = os.path.join(os.path.dirname(__file__), 'court_stat_领馆国际校区.xlsx')
